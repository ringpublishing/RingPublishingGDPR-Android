...
public class CmpWebViewClient extends WebViewClient {
    ...
    public boolean shouldOverrideUrlLoading(WebView view, WebResourceRequest request) {
        if (request.getUrl().getHost() != null && request.getUrl().getHost().contains(BuildConfig.CMP_HOST)) {
            ...
            try {
                ...
                log.error("Activity not found exception when try open link: " + request.getUrl().toString() + " Error: " + e.getLocalizedMessage());
                return false;
            }
        }
    }
